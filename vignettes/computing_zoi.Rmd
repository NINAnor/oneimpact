---
title: "Computing zone of influence rasters for different types of variables"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Computing zone of influence rasters for different types of variables}
...
of nearby features. Zones of influence can be computed for different types of variables,
...
number of infrastructure features in each site within the landscape. Both raster maps from
...
sample landscape in which more than one cabin can be found. In this context,
...
around it. As an illustration, we'll compute here a map representing the count of
...
there is a crossroad of several features or the pixel size is large. In this case, then,
...
or `1-NA` values. If we already have a raster map (e.g. a land use or land cover map),
...
in its surroundings, each metric has a slightly different interpretation:
...
For all types of infrastructure and spatial variables, we recommend users to think 
...
the computation of ZOI metrics might be computationally demanding and time consuming (but 
see the 
...
in order to avoid conclusions based
on unrepresentative ZOI, which can mislead management and conservation 
