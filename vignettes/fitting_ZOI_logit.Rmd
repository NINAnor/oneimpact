...
Estimating the zone of influence (ZOI) of human infrastructure on wildlife requires 
fitting models across a large number of candidate spatial scales (ZOI radii) simultaneously. 
This creates a high-dimensional variable selection problem: with many correlated predictors 
representing the same infrastructure at different radii, standard regression methods 
are unstable and prone to overfitting. Penalized regression addresses this by adding 
a regularization term to the likelihood that shrinks or removes coefficients, possibly 
performing variable selection as part of model fitting. Combined with bootstrap aggregation 
(bagging), which repeatedly fits models on resamples of the data, this approach yields 
robust, uncertainty-aware estimates of both the ZOI radius and the magnitude of 
infrastructure effects on habitat selection.
...
The data was put into a use-availability design, with 1 used location for each 9 random 
...
the cumulative ZOI (density) of infrastructure types. 
...
are aggregated into a bag (a group of models). Each model is based on a different sub-set (a 
...
influence of private cottages and public resorts (columns starting with `cabins_private` and 
`cabins_public`, respectively). Zone of influence variables include both the ZOI of the nearest feature 
...
For simplicity, we perform random sampling here, but the sampling can also be spatially stratified using the `sp_strat` argument. 
...
- `alpha`: The elastic net mixing parameter used in `glmnet::glmnet()`: balances ridge (0) and lasso (1) penalties;
...
are transformed and normalized (sum to 1) according to any standard or user-defined function 
...
In this example, all the models of the bag have a quite good (and comparable) performance,
...
the validation score of the full model. The greater the difference in scores, the greater is the 
...
This vignette demonstrated the full workflow for estimating zones of influence in 
resource selection functions using bagging and penalized regression in `oneimpact`. 
Starting from an annotated GPS dataset, we showed how to define candidate ZOI variables 
across multiple radii and infrastructure types, fit a bag of Adaptive Lasso models, 
evaluate and weight individual model fits, and extract ecologically interpretable 
summaries — including ZOI radii, coefficient profiles, response curves, and spatial 
predictions of infrastructure impact.

The approach is flexible: the penalization method (Lasso, Ridge, Adaptive Lasso and its 
ecology-informed variants), the number of resamples, the train/test/validation split 
strategy, and the evaluation metric can all be adjusted to the specific study system. 
We encourage users to explore these options and refer to Niebuhr et al. (2023) for 
a detailed ecological application and discussion of the method's assumptions and limitations.
...
Niebuhr et al. (2026). Ecology-informed machine learning to estimate the zone of 
influence of multiple disturbances on ecological niche models. *Working manuscript*.
