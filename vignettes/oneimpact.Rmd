...
This is defined by the argument `where`. On the one hand, computations
...
The tool to be used might be determined by the user (argument `module`).
...
The shape of the ZOI might be changed through the argument `type`, using the
functions presented above. This argument might be also set to 
`type = "euclidean"`
...
values might be checked and reclassified to zero using the `zeroAsNA = TRUE` argument,
...
weight matrix might be defined by the user and used as the `radius` argument,
