...
features using different ZOI decay shapes and radii, it is possible to use these different
...
influence is closely linked to the studies on quantity, quality, and fragmentation of suitable habitat and the *scale of effect* of spatial variables on species-habitat relationships (e.g. Miguet et al., 2016; 
...
This is defined by the argument `where`. Computations
...
might become slow for large maps. Alternatively, in GRASS GIS, 
it is possible to compute the ZOI for very large maps using the
software's compiled tools as GRASS GIS does not store maps
...
and may be retrieved to R through the `rgrass7::read_RAST()` function
...
zones of influence. The ZOI functions implemented in `oneimpact` are
shown in Table 1 and can be explored with the command `help(zoi_functions)`. 
The ZOI functions can also be plotted in 1 dimensional space with the `plot_zoi1d()` function,
...
functions. The same ZOI functions can be used in either `calc_zoi_nearest()` 
or `calc_zoi_cumulative()`, accounting
...
These and other weight matrices can be created using the function 
`filter_create()`.
...
The exponential decay can be calculated using the `exp_decay()` function:
...
the `zoi_limit`. By default, `zoi_limit = 0.05`, but this value can be changed by
the user (e.g. to 0.01 or another small value). Changing `zoi_limit` changes the
interpretation of the ZOI radius argument, though:
...
The additional horizontal dashed line at `zoi_limit = 0.05` and vertical dashed
lines at `x = 10` and `x = -10` (since `radius = 10`) illustrate how the ZOI radius
corresponds to the distance where the ZOI reaches `zoi_limit`.
...
The Gaussian decay can be calculated using the `gaussian_decay()` and
...
Notice that, even though the ZOI radius $r$ is defined as 10 for all the decay
functions, the change in their shape strongly modifies the interpretation of how
the ZOI changes with distance. These functions set here can be used to calculate
...
accommodate all these shapes by using the argument `type`. For instance, to compute
...
can be calculated for the zone of influence: the ZOI of the nearest feature alone and the cumulative ZOI of multiple features.

To illustrate the difference between 'nearest' and 'cumulative' ZOI, we will plot the Gaussian decay ZOI
...
the maximum value for the ZOI of the nearest feature is always 1.

Now, we do the same but considering that the ZOI of each feature is additive by
...
Notice that the the maximum value for the cumulative ZOI of multiple features may be greater
...
`calc_zoi_nearest()` and `calc_zoi_cumulative()`. As an example, we present a 
data set with the
location of private cabins in Norway, subset for a small sample region in Southern Norway.
The data is mapped as points in vector format; more information about it can be
...
functions. However, in our case, the data must be rasterized first since it is in vector format. 
For many types of anthropogenic infrastructure or disturbance that are represented
by lines or polygons (e.g. roads, power lines, areas of deforestation), it is sufficient to
...
For point representation of infrastructure and when the raster resolution is coarse 
relative to vector data, it might be more interesting to
count the number of features per pixel, as there may be more than one feature 
within each pixel. To create a raster with the number of
cabins per pixel, we use the function
`terra::rasterize()` with argument `fun = length`. We load another raster with
...
A raster map of ZOI of the nearest feature can be computed using `calc_zoi_nearest()` using a
raster of infrastructure locations as input. For this function,
is it important for the background values of the input raster map (pixels with no cabins)
to be `NA` (no-data). For illustration, we compute the ZOI of the nearest feature using a Gaussian
shaped ZOI with radius = 1000 m. By default, the computation is done in R (argument
`where = "R"`).
...
The shape of the ZOI can be changed through the argument `type`, using the
functions presented above. The `type` argument can be set to `type = "euclidean"` 
for calculation of the Euclidean distance to the nearest feature, 
`"log"` for the log-transformed distance, 
or `"sqrt"` for the square root-transformed distance from the nearest feature.

## Compute the cumulative ZOI of multiple features

In contrast to `calc_zoi_nearest()`, the input raster map for the `calc_zoi_cumulative()`
must use zeros as the background without infrastructure (e.g. pixels with no cabins). In R, background `NA`
values can be checked and reclassified to zero using the `zeroAsNA = TRUE` argument,
however, this is not implemented in GRASS GIS. In this case, the easiest procedure 
...
feature only. Here, the shape of the ZOI can also be changed through the
argument `type`, using the functions presented above. Alternatively, a customized
weight matrix can be defined by the user and used as the `radius` argument,
in case which the user must set `type = "mfilter"`. Other functions to define
weight matrices can be set e.g. through `terra::focalMat()` (Hijmans, 2022) or
...
For `calc_zoi_cumulative()`, the user can choose between computing the 
...
can produce values much greater than one when there are features located closer than the
ZOI (see the illustrations above). The calculation of the density of features, on the other hand,
occurs after a normalization of the weight matrix, so that its values sum to 1.
As a consequence, the density of features generally presents values smaller than 
...
allow the computation of the ZOI of the nearest feature and the cumulative ZOI 
...
Next, we simulate a data set of sampling points and a theoretical random biological 
...
which can compute both ZOI metrics at once. To do so, we set the argument `zeroAsNA`
...
(James et al., 2021). 
These statistical modeling procedures are represented in the other vignettes of `oneimpact`.
...
Alam, M., Skarin, A., & Panzacchi, M. (2023). Estimating the cumulative impact and zone of
