---
title: "Fitting models to estimate the ZOI in a SSF setup using bagging and penalized regression"
format: 
  html:
    self-contained: true
    toc: true
...
# load data
dat <- readRDS("/data/P-Prosjekter/41203800_oneimpact/04_tools/support_oneimpact/moose_test.rds")

# explore columns
colnames(dat)
nrow(dat)
nrow(dat[dat$case_ == 1,])
```
...
f <- case_ ~ strata(step_id_2) + sl_ + log(sl_) + cos(ta_) +
  along_n_crossings_forestry	+
  along_n_crossings_private	+
  along_n_crossings_public	+
  # along_roadvolume_max	+
  along_artype_10	+
  along_artype_20	+
  along_artype_30	+
  along_artype_50	+
  ...
  along_artype_82	+
  along_artype_99	+
  along_forest_resources_SR16_max +
  # along_cabins_private_nose_zoi_cumulative_bartlettXXX_max	+
  # along_cabins_private_nose_zoi_nearest_euclidean_max +
  # along_cabins_public_large_nose_zoi_cumulative_bartlettXXX_max	+
  # along_cabins_public_large_nose_zoi_nearest_euclidean_max	+
  # along_cabins_public_medium_nose_zoi_cumulative_bartlettXXX_max	+
  # along_cabins_public_medium_nose_zoi_nearest_euclidean_max	+
  # along_houses_count_no_max	+
  along_houses_nose_zoi_cumulative_bartlettXXX_max	+
  along_houses_nose_zoi_nearest_euclidean_max	+
  # along_railways_nose_win_zoi_cumulative_bartlettXXX_max	+
  # along_railways_nose_win_zoi_nearest_euclidean_max	+
  # along_roads_major_nose_win_zoi_cumulative_bartlettXXX_max	+
  # along_roads_major_nose_win_zoi_nearest_euclidean_max
  # along_roads_minor_nose_win_zoi_cumulative_bartlettXXX_max	+
  # along_roads_minor_nose_win_zoi_nearest_euclidean_max +
  along_roads_public_nose_win_zoi_cumulative_bartlettXXX_max	+
  along_roads_public_nose_win_zoi_nearest_euclidean_max
# along_type_1	+
# along_type_2	+
# along_type_3	+
# along_type_4	+
...
# along_type_18	+
# along_type_19	+
# along_dem_100m_nosefi_float_TRI100_max	+
# along_dem_100m_nosefi_float_max	+
# along_dem_100m_nosefi_float_diff

# add ZOI terms to the formula
zois <- c(100, 250, 500, 1000, 2500, 5000, 10000)
ff <- add_zoi_formula(f, zoi_radius = zois, pattern = "XXX", 
                      type = "", separator = "", predictor_table = TRUE)

# get formula
f <- ff$formula
# predictor table for usage later to map ZOI-like type variables
predictor_table <- ff$predictor_table

# check variables
all_na <- names(which(apply(dat, 2, function(x) sum(is.na(x))) > 1000))
all_na <- all_na[all_na %in% all.vars(f)]
if(length(all_na) > 1) {
  f <- update.formula(f, as.formula(paste("~ .", all_na, sep = " - ")))
  predictor_table <- predictor_table[grep(all_na, predictor_table$term_zoi, invert = TRUE),]
}
```
...
For SSF, we should use only the presences or used locations in the sampling scheme, and provide the spatial stratification blocks as the 
parameter `sp_strat`. In this case, given the strata names are used to define the blocks, the result of `create_samples()` is going to show
a sequence of strata that should be included in each set, for fitting, calibrating, and testing.

```{r samples}
# sampling - spatially stratified sampling

# explores potential blocks
explore_blocks_pre(dat, colH0 = "studyarea",
                   animal_id = "animal_id",
                   col_case = "case_")

explore_blocks_pre(dat, colH0 = c("studyarea", "year"),
                   animal_id = "animal_id",
                   col_case = "case_")
...
# spatial blocks
blocks <- spat_strat(dat1, coords = c("x1_", "y1_"),
                     colH0 = "studyarea", colID = "step_id_2",
                     block_size = 20000, buffer = max(dat$sl_),
                     plot_grid = TRUE)
...
# sampling
set.seed(123)
samples <- create_resamples(y = dat1$case_,
                            p = c(0.3, 0.2, 0.2),
                            times = 5,
                            sp_strat = blocks, 
...
# if it was random sampling
# samples <- create_resamples(y = dat1$case_,
#                             p = c(0.2, 0.2, 0.2),
#                             times = 3,
#                             sp_strat = NULL,
#                             colH0 = dat1$animal_id)
```

The object `samples` is a list of three elements: a list of sets (defined by the row numbers in the original dataset) that will be used for model fitting (`samples$train`), for variable selection/calibration (`samples$test`), and for model valiation (`samples$validate`). It also contains the the block level
H0 and the strata of each presence in the dataset.
...
# fit multiple models
tic()
fittedl <- bag_fit_net_clogit(f, dat,
                              samples = samples,
...
bag_summary <- bag_models(fittedl, dat, 
                          weights_col = "habitat_validation_score",
...
# variable importance
importance <- variable_importance(bag_summary, dat, samples, 
                                  type = "drop",
                                  order = "asc")
# metric = conditionalBoyce)
...
# different plots
plot_coef(bag_summary, terms = "long_houses_nose_zoi_cumulative")
plot_coef(bag_summary, terms = "long_houses_nose_zoi_cumulative", 
          plot_type = "histogram")
plot_coef(bag_summary, terms = "long_houses_nose_zoi_cumulative", 
          plot_type = "points")
# in only one or few models
plot_coef(bag_summary, terms = "long_houses_nose_zoi_cumulative", models = 1:3)
```
...
plot_coef(bag_summary, what = "average", terms = "roads", 
          plot_type = "points")
...
dfvar = data.frame(along_n_crossings_forestry = seq(min(bag_summary$data_summary$along_n_crossings_forestry),
                                                    max(bag_summary$data_summary$along_n_crossings_forestry),
                                                    length.out = 100))
# reference median
...
The adaptive Lasso if fitted using the parameter `method = "DecayAdaptiveLasso"` in `fit_net_logit` and `bag_fit_net_logit`. An extra parameter is needed here: a `predictor_table` matrix stating whether a each of the terms in the formula is or not a ZOI variable and the radius of the ZOI. We use the variable grid `grid_zoi` which is also output of the `add_zoi_formula()` function.
