...
ff <- add_zoi_formula(f, zoi_radius = zois, pattern = "XXX", 
                      type = "", separator = "", grid = TRUE)

# get formula
f <- ff$formula
# grid for usage later to map ZOI-like type variables
grid_zoi <- ff$grid
...
if(length(all_na) > 1) {
  ...
  grid_zoi <- grid_zoi[grep(all_na, grid_zoi$term_zoi, invert = TRUE),]
}
```
...
blocks <- spat_strat(dat1, coords = c("x1_", "y1_"),
                     ...
                     block_size = 10000, buffer = max(dat$sl_),
                     plot_grid = TRUE)
...
The adaptive Lasso if fitted using the parameter `method = "DecayAdaptiveLasso"` in `fit_net_logit` and `bag_fit_net_logit`. An extra parameter is needed here: a grid matrix stating whether a each of the terms in the formula is or not a ZOI variable and the radius of the ZOI. We use the variable grid `grid_zoi` which is also output of the `add_zoi_formula()` function.

```{r fit1model_dal}
mod <- fit_net_logit(f, dat, samples, i = 1, metric = proc_AUC,
                     method = "DecayAdaptiveLasso",
                     predictor_grid = grid_zoi)
```
