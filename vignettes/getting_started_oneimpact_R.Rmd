...
Infrastructure development often takes place in landscapes already affected by 
multiple anthropogenic disturbances, leading to cumulative impacts on biodiversity. 
Typically, the impact of a given type of infrastructure is determined by computing 
...
impact assessments. In version 0.1.0, its main functions are related to the
...
widely explored in the landscape ecology literature (Miguet et al., 2016; 
...
spatial decay of the ZoI, and `zoi_radius`, which defines how far the influence
expands in space (or how fast it decreases with distance). 

Both functions can be run in R environment (R Core Team, 2021) and in GRASS GIS 
...
software's algorithms, and given that GRASS GIS does not store maps
...
name of the output map. This map is stored in the the GRASS GIS location/mapset, 
and might be retrieved to R through the `rgrass::read_RAST()` function
or exported outside GRASS using the `r.out.gdal` module, for instance.
...
shown in Table D1 and might be explored with the command `help(zoi_functions)`. 
They might also be plotted in 1 dimensional space with the `plot_zoi1d()` function,
for illustration of their behavior.

Table D1: Main functions in `oneimpact` used to calculate zones of influence. They are 
divided in three types: zone of influence functions ("ZoI functions"), functions to 
compute the ZoI for raster maps ("Compute the ZoI"), and functions to create filters
...
| Compute ZoI | `calc_zoi()`  | This function takes in a raster with locations or counts of infrastructure and calculates a raster with either or both zone of influence metrics: the ZoI of the nearest feature only and the cumulative ZoI. |
| Create filters | `create_filter()` | This function creates matrices of weights following different functions to be used in neighborhood analyses for rasters. In the context of cumulative impact analysis, they represent the Zone of Influence (ZoI) of each infrastructure point/pixel, to be used to calculate the cumulative ZoI or density of features. |
| Create filters | `save_filter()` | This function saves a matrix with weights (filter or kernel matrix) in an external text file. It can save either the raw matrix or save a file using the standards for running the `r.mfilter` algorithm within GRASS GIS. |

</center>

The cumulative ZoI of the multiple features (the output of `calc_zoi_cumulative()`) 
...
through the `terra::focal()` function in R and through one 
of the modules in GRASS GIS: `r.mfilter`, `r.resamp.filter`, or `r.neighbors`. 
The module to be used might be determined by the user (parameter `module`).

# The concept of zone of influence

The zone of influence (ZoI) is the function $\phi$ that informs how the 
impact of a given infrastructure feature or source of disturbance decreases with distance. 
Formally, the 
...
same functions (e.g. `linear_decay()` and `bartlett_decay()`).
...
```{r plot_threshold, message=FALSE, error=FALSE, warning=FALSE, fig.cap="Illustration of a threshold decay ZoI with ZoI radius = 10."}
# threshold ZoI with radius = 10
plot_zoi1d(points = 0, zoi_radius = 10, fun = threshold_decay, range_plot = c(-20, 20))
```
...
```{r plot_linear, message=FALSE, error=FALSE, warning=FALSE, fig.cap="Illustration of a linear decay ZoI with ZoI radius = 10."}
# threshold ZoI with radius = 10
plot_zoi1d(points = 0, zoi_radius = 10, fun = linear_decay, range_plot = c(-20, 20))
```
...
As it is possible to see, an exponential decay ZoI with $\verb|zoi_radius = 10|$ does
not imply the function is null beyond the ZoI radius, but the it goes below
the `zoi_limit`. By default, $\verb|zoi_limit = 0.05|$, but this value might be changed by
...
```{r plot_exp, message=FALSE, error=FALSE, warning=FALSE, fig.cap="Illustration of an exponential decay ZoI with ZoI radius = 10 and ZoI limit = 0.05."}
# threshold ZoI with radius = 10
...
We add to the plot a horizontal dashed line at $\verb|zoi_limit = 0.05|$ and vertical dashed
lines at $x = 10$ and $x = -10$ ($\verb|zoi_radius = 10|$), to show that the ZoI radius
represent the distance where the ZoI reaches `zoi_limit`.
...
```{r plot_gauss, message=FALSE, error=FALSE, warning=FALSE, fig.cap="Illustration of a Gaussian decay ZoI with ZoI radius = 10 and ZoI limit = 0.05."}
# threshold ZoI with radius = 10
...
ZoI of multiple features.

## ZoI metrics

Given a ZoI function was set with a specific shape and ZoI radius, two ZoI metrics
might be calculated: the ZoI of the nearest feature only and the cumulative ZoI of
multiple features.
...
the ZoI radius of each feature as $\verb|zoi_radius = 3|$. We start by plotting 
the ZoI of the nearest feature alone by setting $\verb|cumulative = FALSE|$:

```{r zoi_metrics_near, fig.cap="Metric of ZoI of the neareast feature for multiple sources of disturbance illustrated in 1 dimension using a Gaussian decay ZoI with radius = 3."}
...
setting $\verb|cumulative = TRUE|$ in `plot_zoi1d()`:

```{r zoi_metrics_cum, fig.cap="Metric of cumulative ZoI for multiple sources of disturbance illustrated in 1 dimension using a Gaussian decay ZoI with radius = 3."}
plot_zoi1d(points = disturbance_locations, zoi_radius = 3, fun = gaussian_decay,
...
# Calculating the ZoI for rasters
...
data set with the
location of private cabins in Norway, subset for a small study region in Southern Norway.
The data is mapped as points in vector format; more information about it might be
found using the command `help(cabins_vect.gpkg)`. We read the vector file using the
package `terra`:
...
`terra::rasterize()` with parameter $\verb|fun = length|$. We load another raster with
...
values might be checked and reclassified to zero using the $\verb|zeroAsNA = TRUE|$ parameter,
...
weight matrix might be defined by the user and used as the `zoi_radius` parameter,
in case which the user must set $\verb|type = "mfilter"|$. Other functions to define
...
cumulative ZoI metric when
$\verb|output_type = "cumulative_zoi"|$
(default), or the density of features if $\verb|output_type = "density"|$. The cumulative
